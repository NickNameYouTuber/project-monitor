...
const FileViewer: React.FC<FileViewerProps> = ({ repositoryId, file }) => {
  ...
  if (!file) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-[var(--bg-secondary)] rounded-lg shadow-sm border border-[var(--border-primary)] overflow-hidden">
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-full">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[var(--border-primary)]"></div>
      </div>
    );
  }
  ...
  if (fileContent.binary) {
    if (fileContent.name.match(/\.(jpg|jpeg|png|gif|bmp|svg|webp)$/i)) {
      return (
        <div className="flex flex-col h-full bg-[var(--bg-secondary)] rounded-lg shadow-sm border border-[var(--border-primary)] overflow-hidden">
          <div className="p-2 border-b border-[var(--border-primary)] flex justify-between items-center bg-[var(--bg-header)] flex-wrap gap-2">
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full bg-[var(--bg-secondary)] rounded-lg shadow-sm border border-[var(--border-primary)] overflow-hidden">
        ...
        <p className="text-[var(--text-muted)] text-sm mt-1">File type: {fileContent.name.split('.').pop()}</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-secondary)] rounded-lg shadow-sm border border-[var(--border-primary)] overflow-hidden">
      ...
      <div className="flex-grow overflow-auto bg-[var(--bg-tertiary)] text-[var(--text-primary)] font-mono text-sm">
        <SyntaxHighlighter 
          language={language} 
          style={vscDarkPlus} 
          customStyle={{ margin: 0, height: '100%', background: 'var(--bg-tertiary)' }}
          showLineNumbers
        >
          {fileContent.content}
        </SyntaxHighlighter>
