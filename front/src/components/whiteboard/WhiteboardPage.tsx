...
function WhiteboardPage({ projectId }: WhiteboardPageProps) {
          ...
          return (
            ...
            >
              <button 
                ...
                className={`flex items-center justify-center w-10 h-10 rounded-full shadow-md transition-all duration-200 border-2 ${
                  c.isOpen 
              ...
              {c.isOpen && (
                <div 
                    className={`w-80 rounded-xl shadow-2xl border flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-left ${
                        isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
                    }`}
                    onPointerDown={(e) => e.stopPropagation()}
                >
                    {/* Header */}
                    <div className={`px-4 py-3 border-b flex justify-between items-center ${isDarkMode ? 'border-gray-700 bg-gray-900/50' : 'border-gray-100 bg-gray-50/50'}`}>
                        <div className="flex items-center gap-2">
                            <div className={`w-2 h-2 rounded-full ${isDarkMode ? 'bg-blue-400' : 'bg-blue-500'}`}></div>
                            <span className={`text-xs font-bold uppercase tracking-wide ${isDarkMode ? 'text-gray-300' : 'text-gray-500'}`}>Comments</span>
                        </div>
                        <button 
                    ...
                    <div className="max-h-64 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                        ...
                        {c.messages.map((m, i) => (
                            <div key={m.id} className="flex flex-col gap-1 animate-in slide-in-from-bottom-2 duration-300 fill-mode-backwards" style={{ animationDelay: `${i * 50}ms` }}>
                                <div className={`p-3 rounded-2xl rounded-tl-none text-sm leading-relaxed shadow-sm ${
                                    isDarkMode 
                    ...
                    <div className={`p-3 border-t ${isDarkMode ? 'border-gray-700 bg-gray-900/30' : 'border-gray-100 bg-gray-50/50'}`}>
                        <div className={`flex items-center gap-2 rounded-lg border px-3 py-2 transition-ring focus-within:ring-2 focus-within:ring-blue-500/20 ${
  ...
  return (
    <div className={`w-full h-full relative overflow-hidden flex flex-col ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <div className="absolute inset-0 pointer-events-none z-50">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-20 flex items-center justify-start pointer-events-none">
                 {!isMobile && (
      ...
      <div 
        ...
        // onWheel handler removed here, now handled by native listener in useEffect
      >
        ...
        {/* Zoom Indicator */}
        <div className={`absolute bottom-4 right-4 border shadow-md rounded-md px-3 py-1 text-sm font-medium ${isDarkMode ? 'bg-gray-800 border-gray-700 text-gray-200' : 'bg-white border-gray-200 text-gray-600'}`}>
            {Math.round(scale * 100)}%
        </div>

        {/* Mobile Banner */}
        {isMobile && (
            <div className="absolute bottom-16 left-4 right-4 bg-blue-600 text-white p-3 rounded-lg shadow-lg text-center text-sm font-medium">
